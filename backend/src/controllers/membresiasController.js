...
const create = (req, res) => {
    try {
        ...
        const membresia = db.prepare('SELECT * FROM membresias WHERE id = ?').get(result.lastInsertRowid);
...
const remove = (req, res) => {
    try {
        ...
        const membresia = db.prepare('SELECT id FROM membresias WHERE id = ?').get(id);
        if (!membresia) {
            return res.status(404).json({ error: 'Membership not found' });
        }
